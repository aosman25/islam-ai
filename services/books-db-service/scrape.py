...
def process_book_html_metadata_only(
    html_contents: List[str],
    book_id: int,
    book_name: str,
    author_name: Optional[str],
    ...
    """
    Process HTML files from an exported book and create metadata only (no text file).

    This is a more efficient version when text output is not needed.
    ...
    # Process pages for metadata extraction
    for html in html_contents:
        soup = BeautifulSoup(html, "html5lib")
        main_div = soup.find("div", class_="Main")
        if not main_div:
    ...
    # Build metadata (without text content)
    metadata = {
        "book_id": book_id,
        "book_name": book_name,
        "author": author_name,
    ...
    logger.info(
        "Processed book HTML (metadata only)",
        book_id=book_id,
        total_headers=len(headers),
        total_pages=sum(len(pages[h]) for h in pages)
    )
