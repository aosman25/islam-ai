...
export function Footer() {
  return (
    <footer className="border-t border-border/60 bg-parchment-100/50">
      <div className="mx-auto max-w-7xl px-page py-16">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
          ...
          <div className="space-y-4">
            ...
            <p className="font-arabic text-base text-ink-400">
          ...
          {FOOTER_LINKS.map((group) => (
            <div key={group.title}>
              <h3 className="font-display text-sm font-semibold text-ink-700 uppercase tracking-wider mb-4">
                {group.title}
              </h3>
              <ul className="space-y-3">
                {group.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="flex items-center gap-2 text-sm text-ink-500 hover:text-gold-700 transition-colors"
                    >
